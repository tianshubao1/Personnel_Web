...
function initialise_document_ready_functions()
{
	...
	var section_ids = ["#home", "#aboutme", "#portfolio", "#contact", "#blog"];
	...
	*/
	...
	$(window).mousewheel(function(event) {
		if (!$("#main-content").hasClass("same_page_link_in_action") && !jQuery.browser.mobile ){
			...
			var active_section_id = get_current_section_id(section_wrappers, amount_of_pixels_as_buffer_between_sections);
			var target_id_index = section_ids.indexOf(active_section_id);
			if(event.deltaY < 0)   //down
				target_id_index++;
			else		//up
				target_id_index--;
			if(target_id_index>=0 && target_id_index<section_ids.length){
				scroll_to_section(section_ids[target_id_index], "menu-item-"+section_ids[target_id_index].substring(1), false);
			}
			return false;
		}
